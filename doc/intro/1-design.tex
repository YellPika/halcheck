...
\begin{frame}[fragile,t]{\halcheck{} \textemdash{} Design \textemdash{} \mintinline{c++}{gen::next}}
  \begin{center}
  ...
  \begin{onlyenv}<+-+(2)>
    \begin{columns}[T]
      ...
      \begin{itemize}
        \item<.(1)-> Solution 1: consult \mintinline{c++}{gen::range} to determine which value to return.
        \item<.(2)-> Solution 2: define \mintinline{c++}{gen::element} (implemented via \mintinline{c++}{gen::range}).
      \end{itemize}
    ...
    \begin{columns}[T]
      ...
      \begin{onlyenv}<.(1)-.(2)>
        \begin{itemize}
          ...
          \item<.(2)-> Problem: \mintinline{c++}{gen::arbitrary<size_t>} is uniformly distributed \textemdash{} high chance of generating \emph{billions of GBs}.
        \end{itemize}
      \end{onlyenv}

      \begin{onlyenv}<.(3)-.(4)>
        \begin{itemize}
          \item<.(3)-> Attempt: add elements as long as \mintinline{c++}{gen::next} returns \mintinline{c++}{true}.
...
\begin{frame}[fragile,t]{\halcheck{} \textemdash{} Design \textemdash{} \mintinline{c++}{gen::next}}
  ...
  \begin{itemize}
    ...
    \item A \mintinline{c++}{gen::weight} is a symbolic value parameterized by an unknown size.
    \item A \mintinline{c++}{gen::weight} is semantically a function in $\mathbb{N} \to \mathbb{N}$.
...
\begin{frame}[fragile]{\halcheck{} \textemdash{} Design \textemdash{} \mintinline{c++}{gen::next}}
...
\begin{frame}[fragile]{\halcheck{} \textemdash{} Design \textemdash{} \mintinline{c++}{gen::discard}}
  \begin{center}
  ...
  \begin{itemize}
    \item Used to implement precondition checking (\mintinline{c++}{RC_PRE}).
    \item Also useful for implementing rejection sampling (\mintinline{c++}{gen::suchThat}).
  \end{itemize}
\end{frame}

\begin{frame}[fragile]{\halcheck{} \textemdash{} Design \textemdash{} \mintinline{c++}{gen::shrink}}
  \begin{center}
...
\begin{frame}[fragile,t]{\halcheck{} \textemdash{} Design \textemdash{} \mintinline{c++}{gen::group}}
  \begin{center}
...
\begin{frame}{\halcheck{} \textemdash{} Design \textemdash{} Core}
  ...
  \begin{itemize}
    ...
    \item<+-> Effects are \emph{invoked} via the call operator (\mintinline{c++}{effect()}).
    \item<+-> A \emph{handler} can be \emph{installed} using \mintinline{c++}{auto _ = effect.handle(my_handler)} is \emph{uninstalled} when the return value (\mintinline{c++}{_}) goes out of scope.
