...
\begin{frame}[fragile,t]{\halcheck{} --- Motivation --- API}
  ...
  \begin{onlyenv}<+>
    ...
    \textbf{Problem:} \mintinline{c++}{*gen::inRange(0, N)} is evaluated \alert{before} \mintinline{c++}{example} returns, resulting in a generator that always produces \mintinline{c++}{std::vector}s of the same length.
  \end{onlyenv}
