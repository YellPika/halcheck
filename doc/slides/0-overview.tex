...
\begin{frame}[fragile,t]{\halcheck{} --- Overview --- API}
  \textbf{Example:} \emph{Write a generator combinator that produces \cppinline{std::vector}s shorter than a given length.}
...
\begin{frame}[fragile]{\halcheck{} --- Overview --- Space Complexity}
  \begin{columns}[t,onlytextwidth]
    ...
    \begin{block}{Shrink tree for a list:}
      \begin{tikzpicture}[align=center,anchor=north,level distance=1.05cm]
        \node{"100"}
        child {node {"00"\\\vdots}}
        child {node {"10"\\\vdots}}
        child {node {"10"}
