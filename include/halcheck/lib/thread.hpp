...
template<typename ThreadID, typename State>
struct serializability_monitor {
public:
  static_assert(lib::is_hashable<ThreadID>(), "ThreadID should be hashable");
  static_assert(lib::is_copyable<State>(), "State should be copyable");
...
private:
  ...
  bool check(State seed, std::vector<std::size_t> &references, std::vector<const_iterator> &queue) const {
...
void lib::serializability_monitor<ThreadID, State>::invoke(R range, F func) {
